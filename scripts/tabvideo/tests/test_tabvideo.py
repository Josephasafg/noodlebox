...
def test_the_struck_note_of_a_pair_takes_the_column_its_figure_is_set_in() -> None:
    """
    A legato figure is set with its *last* fret in the column, the rest hanging left.

    Measured over the reference clip, a lone number printed on a neighbouring
    string inside a figure's box sits at the centre of that last fret every time.
    But the note that is struck is the figure's first fret, so leaving the frets
    where they are drawn puts the chord alongside the wrong one: a strum of
    `7655` whose top string hammers to 7 came out as a lone 5, then a chord of
    765 with the 7 in it.
    """
    run = _run_of(["5", "7"])
    texts, _ = _emit_one("57", ["5", "7"])
    struck, hammered = (t for t in texts if t.str.isdigit())
    # Where the `7` of `57` is printed, which is where a neighbour would be set.
    column = run.x0 + (run.x1 - run.x0) * 0.75
    assert struck.x + struck.width / 2 == column
    assert hammered.x + hammered.width / 2 > column
